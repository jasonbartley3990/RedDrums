...
class HighHatClosePlayer: NSObject, AVAudioPlayerDelegate {
    static let HighHat = HighHatClosePlayer()
                    ...
                    if player3?.isPlaying == true {
                        if player4?.isPlaying == true {
                            if player5?.isPlaying == true {
                                player6 = try AVAudioPlayer(contentsOf: URL(fileURLWithPath: musicUrl))
