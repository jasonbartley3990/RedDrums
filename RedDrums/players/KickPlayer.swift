...
class KickPlayer: NSObject, AVAudioPlayerDelegate {
    static let kick = KickPlayer()
                            ...
                            if player5?.isPlaying == true {
                                if player6?.isPlaying == true {
                                    if player7?.isPlaying == true {
                                        player8 = try AVAudioPlayer(contentsOf: URL(fileURLWithPath: musicUrl))
                                        play(audioPlayer: player8)
                                    } else {
                                        player7 = try AVAudioPlayer(contentsOf: URL(fileURLWithPath: musicUrl))
                                        play(audioPlayer: player7)
                                    }
                                } else { player6 = try AVAudioPlayer(contentsOf: URL(fileURLWithPath: musicUrl))
                                    play(audioPlayer: player6)
                                }
                            } else {
