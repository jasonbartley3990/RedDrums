...
struct CreateBeatMenu: View {
    ...
    @State var beat: Beat?
    @State private var showingNewNoteScreen = false
    @State private var showingNewBeatAlert = true
        ...
        GeometryReader { geo in
            ZStack {
                ...
                VStack {
                    ...
                    NewBeatNameAlert(isShown: $showingNewBeatAlert, text: $newBeatName, onDone: {
                        name in
                        createBeat(name: name)
                        self.showingNewBeatAlert = false
                        
                    }, onCancel: {
                        presentationMode.wrappedValue.dismiss()
                    }).opacity(showingNewBeatAlert ? 1 : 0).environment(\.managedObjectContext, context).clipShape(RoundedRectangle(cornerRadius: 20))
                    Spacer()
                }
    ...
    func createBeat(name: String) {
        let newBeat = Beat(context: self.context)
        newBeat.name = name
        newBeat.id = UUID()
...
struct CreateBeatMenu_Previews: PreviewProvider {
